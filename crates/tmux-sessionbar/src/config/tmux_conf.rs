...
pub fn generate(config: &Config, _binary_path: &str) -> String {
    ...
    let ai_status_path = format!("{home}/.config/tmux-sessionbar/bin/tmux-ai-status");
    ...
    if config.pane_border.enabled {
        ...
        let pane_ai = if pb.show_ai_status {
            format!(
                " #[fg={}]#({} pane '#{{pane_id}}')",
                pb.ai_fg, ai_status_path
            )
        } else {
            String::new()
        };
        let format = format!(
            "\
             #{{?pane_active,#[fg={abg} bold] {pane_id} #[fg={ifg} nobold]#{{s|{home}|~|:pane_current_path}} (#{{{pid}}}){pane_ai}#[default],\
             #[fg={ifg}] {pane_id}{pane_ai} #[default]\
             }}",
            pid = "pane_pid",
            abg = pb.active_bg,
            ifg = pb.inactive_fg,
...
fn render_right_segment(blocks: &[String], config: &Config) -> String {
    ...
    for block in blocks {
        match block.as_str() {
            "ai-window" => {
                let text = format!(
                    " #({home}/.config/tmux-sessionbar/bin/tmux-ai-status window '#{{session_name}}:#{{window_index}}') "
                );
                parts.push(tmux_fmt::conf_style(
                    &config.blocks.ai_window.fg,
                    &config.blocks.ai_window.bg,
                    false,
                    &text,
                ));
            }
...
mod tests {
    ...
    #[test]
    fn right_segment_can_render_ai_window_block() {
        let mut config = default_config();
        config.status.right.blocks = vec!["ai-window".into()];
        let out = make(&config);
        assert!(out.contains("tmux-ai-status window"));
        assert!(out.contains("#{session_name}:#{window_index}"));
    }

    #[test]
    fn pane_border_can_render_ai_status() {
        let mut config = default_config();
        config.pane_border.show_ai_status = true;
        let out = make(&config);
        assert!(out.contains("tmux-ai-status pane"));
        assert!(out.contains("#{pane_id}"));
    }
