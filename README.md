...
2. **`tmux-windowbar apply`** (`run-shell -b`) — registers mouse click/double-click bindings, writes click handler scripts to `/usr/local/bin/`, sets up window/pane event hooks
3. **`tmux-sessionbar render-status left`** (`run-shell`) — populates session list in status bar
...
| Click handlers | `/usr/local/bin/tmux-click-handler` | Generated by `tmux-windowbar apply` at runtime |
